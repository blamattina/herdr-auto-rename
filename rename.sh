...
agent_prompt="Write a 2-4 word, verb-first label for what this coding agent is doing right now. Start with a present-participle verb (e.g. 'Fixing SSH proxy config', 'Rewriting commit history', 'Debugging push failure'). Capitalize only the first word; keep acronyms uppercase. Stay under ${MAX_LABEL_LENGTH} characters. Output ONLY the label — no punctuation, no quotes, no explanation:

${pane_output}"

agent_name=$(bash -c "$GENERATOR \"\$1\"" -- "$agent_prompt" 2>/dev/null | head -1 | tr -d "\"'" | sed 's/^[[:space:]]*//' | cut -c1-"${MAX_LABEL_LENGTH}" || true)
...
if [ ! -f "$ws_lock" ] && [ -n "$workspace_id" ]; then
  touch "$ws_lock"
  ws_prompt="Write a 2-4 word, verb-first label for the overall goal of this coding session (e.g. 'Tuning rename labels', 'Setting up CI'). Capitalize only the first word; keep acronyms uppercase. Stay under ${MAX_LABEL_LENGTH} characters. Output ONLY the label — no punctuation, no quotes, no explanation:

${pane_output}"

  ws_name=$(bash -c "$GENERATOR \"\$1\"" -- "$ws_prompt" 2>/dev/null | head -1 | tr -d "\"'" | sed 's/^[[:space:]]*//' | cut -c1-"${MAX_LABEL_LENGTH}" || true)
